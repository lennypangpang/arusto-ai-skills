...
CATEGORY_KEYWORDS: dict[str, list[str]] = {
    "TECHNOLOGY": [
        ...
        "fullstack", "devops", "cloud engineer", "platform engineer", "site reliability",
        "mobile developer", "ios developer", "android developer", "systems engineer",
        "network engineer", "security engineer", "cybersecurity", "infrastructure engineer",
        "embedded", "firmware", "it engineer", "it specialist", "programmer",
    ],
    "DATA-ANALYTICS": [
        "data analyst", "data scientist", "data engineer", "analytics engineer",
        "business analyst", "business intelligence", "bi developer", "reporting analyst",
        "quantitative analyst", "research analyst", "data architect",
    ...
    "MARKETING": [
        ...
        "seo", "sem", "content strategist", "content writer", "brand manager",
        "digital marketing", "social media manager", "growth manager",
        "campaign manager", "communications manager", "public relations",
        "copywriter", "creative director", "email marketing",
    ],
    "SALES": [
        "sales representative", "sales rep", "account executive",
        "business development representative", "bdr", "sdr", "sales development",
        "sales engineer", "solutions engineer", "inside sales", "outside sales",
        "territory manager", "regional sales manager", "sales manager", "sales director",
    ],
    "FINANCE": [
        "financial analyst", "finance analyst", "accountant", "accounting manager",
        "controller", "fp&a", "financial planning", "auditor", "tax analyst",
        "treasury analyst", "investment analyst", "portfolio manager",
        "risk analyst", "credit analyst", "actuary", "finance manager",
        "chief financial",
    ],
    "HR-OPERATIONS": [
        "human resources", "hr manager", "hr generalist", "hr business partner",
        "hr coordinator", "recruiter", "talent acquisition", "people operations",
        "operations manager", "operations analyst", "supply chain", "logistics manager",
        "procurement", "warehouse manager", "fulfillment manager",
        "compensation", "payroll",
    ],
    "PRODUCT-DESIGN": [
        "product manager", "product owner", "ux designer", "ui designer",
        "ux researcher", "product designer", "user experience", "user interface",
        "interaction designer", "visual designer", "graphic designer",
        "product lead", "head of product",
    ],
    "CUSTOMER-SUCCESS": [
        "customer success", "customer support", "customer service manager",
        "client success", "client services", "support engineer", "technical support",
        "help desk", "customer experience", "service desk", "cx manager",
    ],
}


def load_postings(path: str, sample_n: int) -> pd.DataFrame:
    # nrows takes the first N rows — not a random sample. CSV is roughly chronological so recency bias exists.
    ...
    processed        = pd.to_datetime(df["last_processed_time"], dayfirst=True, format="mixed", utc=True)
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["skills_norm"] = out["skills_list"].apply(
        lambda xs: [re.sub(r"\s+", " ", str(x).strip()).lower() for x in xs if str(x).strip()]
    )
