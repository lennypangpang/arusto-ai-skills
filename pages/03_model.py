...
if not os.path.exists(MODEL_PATH):
    st.warning("No trained model found. Run `python models/train.py` locally to generate `assets/models/baseline.pkl`.")
    st.stop()
...
with st.expander("Model Evaluation", expanded=False):
    ...
    sns.heatmap(cm_df, annot=True, fmt="d", cmap="BuPu", linewidths=0.5, linecolor="white", ax=ax)
